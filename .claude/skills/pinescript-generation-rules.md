...
- A toggle (bool input) called "Show Subwaves" to show/hide the subwave lines and labels (primary count only); default value is `true`
  ...
  - **Subwave label overlap at coincident pivots (e.g. W3.sw5 = W3 same timestamp): no fully satisfactory zoom-independent solution exists in Pine Script v6. Leave subwave labels at exact pivot price using normal style_label_down/up with no offsets. The size.tiny vs size.small distinction provides some visual differentiation.**
