...
**NO SCRIPTS RULE: Do not write or execute any scripts (Python, PowerShell, Bash, or any shell language) for analysis, data fetching, or calculations at any point. All data fetching must use the WebFetch tool directly against the Yahoo Finance API. All calculations (Fibonacci, pivot identification, retracement percentages) must be performed in-context using the model's own reasoning. Running scripts to produce intermediate output that is then read back is forbidden — it produces chat output and wastes tokens.**
