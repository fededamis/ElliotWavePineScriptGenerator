...
- **NO SEMICOLON STATEMENT CHAINING**: Pine Script v6 does not support `;` as a statement separator. Each statement must be on its own line. Scan for any line containing `; array.` or `; line.` or `; label.` patterns and split them into separate lines.
